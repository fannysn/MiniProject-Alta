...
WebUI.callTestCase(findTestCase('WEB UI/Dummy/Login'), [:], FailureHandling.STOP_ON_FAILURE)

WebUI.delay(5)

WebUI.navigateToUrl('https://qa.alta.id/transaction')

WebUI.delay(5)

xxx = WebUI.getText(findTestObject('WEBUI-Object/TransactionPage/div_1-10 of 21'))

def record = xxx.tokenize(' ').last()
record = 0
...
WebUI.verifyElementText(findTestObject('WEBUI-Object/TransactionPage/TransactionsHeader'), 'Transactions')

WebUI.refresh()

WebUI.delay(5)

yyy = WebUI.getText(findTestObject('WEBUI-Object/TransactionPage/div_1-10 of 21'))

def record2 = yyy.tokenize(' ').last()

WebUI.verifyLessThan(record, record2)
